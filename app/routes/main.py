...
@main_bp.route('/mensajes/sala/<string:sala>', methods=['GET'])
@login_required
def obtener_mensajes_por_sala(sala):
    """Obtener historial de mensajes por nombre de sala (p.ej. paciente_1, examen_5, cita_12)"""
    try:
        user_id = session.get('user_id')
        user_rol = session.get('rol')

        # Si la sala es del tipo paciente_<id>, aplicamos la misma regla de acceso.
        if sala.startswith('paciente_'):
            try:
                paciente_id = int(sala.split('_', 1)[1])
            except Exception:
                return jsonify({'success': False, 'error': 'Sala inválida'}), 400
        ...
        # Si la sala es del tipo examen_<id> o cita_<id>, verificamos relación con examen
        elif sala.startswith('examen_') or sala.startswith('cita_'):
            try:
                examen_id = int(sala.split('_', 1)[1])
            except Exception:
                return jsonify({'success': False, 'error': 'Sala inválida'}), 400
            ...
            mensajes = Mensaje.query.filter_by(sala=sala).order_by(Mensaje.fecha.asc()).all()

        else:
            # Sala genérica: solo técnicos pueden consultarla
            if user_rol != 'Tecnico':
                return jsonify({'success': False, 'error': 'No autorizado'}), 403
            mensajes = Mensaje.query.filter_by(sala=sala).order_by(Mensaje.fecha.asc()).all()
        ...
        return jsonify({'success': True, 'mensajes': mensajes_list}), 200
